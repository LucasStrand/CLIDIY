	...
	.action((opts) => {
		...
		if (opts.set) {
			for (const kv of opts.set as string[]) {
				...
				if (k !== 'clientId' && k !== 'tenantId') throw new Error(`Unknown key: ${k}`);
				(cfg as any)[k] = v;
			}
			writeConfig(cfg);
...
program
	...
	.action(async (opts) => {
    const data: any = await listChannels(opts.team);
...
program
	...
	.action(async (opts) => {
    const res: any = await sendChannelMessage(opts.team, opts.channel, opts.message);
